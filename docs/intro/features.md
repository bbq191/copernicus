...
| 会议纪要 | 标题 + 按模板排版的纪要（5 种模板可选），以及单独列出的**行动项与决议**（点击时间跳到录音对应位置） | 自动完成，可换模板重新生成 |
...
**它不做什么**：不做实时转写；没有用户与权限体系（请放在内网或网关之后）；合规审核只有转写文本和画面文字两路证据，人脸检测结果目前只保存、不参与判定；纪要正文是自由文本，行动项与决议的提取依赖大模型，质量尚未用真实模型评估。
...
| 关键帧提取 | 每隔约 2 秒截一张图（最多 500 张；很长的视频会自动拉宽间隔），后面 OCR 和人脸检测要用 |
...
- **行动项与决议**：排版纪要之外单独提取，列在纪要下方。每条带原文中的位置，点击时间即可跳转到录音。模型没能在转写里找到出处的条目不显示时间。提取失败或不全时会有提示，未列出不代表会上没有。
...
| 取消 | 排队、等待语音识别、文本纠正、生成纪要、合规审核阶段可取消；正在语音识别与视觉扫描时不可取消 |
