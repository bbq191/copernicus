...
> **适用版本说明（2026-09）**：本笔记的代码片段截取自 2026 年 5 月前后的源码。此后前端做过大规模重构（任务级取消范围、复核持久化、按 id 选中、播放同步改为按需 rAF、轮询后台降频等），部分文件与函数已改名或删除。**请把它当作 React/TypeScript 概念教程阅读，具体实现以当前源码为准**；当前架构见 `docs/intro/frontend-architecture.md`。
