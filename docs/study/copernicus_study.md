...
> **适用版本说明（2026-09）**：本笔记的代码片段截取自 2026 年 5 月前后的源码。此后项目做过多轮重构——`llm.py` 已拆成 `services/llm/` 包，`task_store.py` 拆出了 `task_state.py`，日志、部署、模型管理、上传与合成链路都有较大调整，部分函数已被删除或改名。**请把它当作 Python 概念教程阅读，具体实现以当前源码为准**；当前架构见 `docs/intro/backend-architecture.md`。
