...
import { synthesizeTask, getSynthesisAudioUrl } from "../../api/synthesis";
...
export function SynthesisPanel() {
  ...
  const setHasSynthesis = useSynthesisStore((s) => s.setHasSynthesis);
  ...
  const handleSynthesize = async () => {
    ...
    setCurrentTime(0);
    try {
      const res = await synthesizeTask(taskId);
      setResult(res.duration_ms, res.synthesis_time_ms);
      // 重新合成后强制 audio 重新加载同名 URL 的新内容
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
          ...
          <div className="bg-base-200 rounded-xl p-3 flex flex-col gap-2">
            ...
            {(durationMs !== null || synthesisMs !== null) && (
              <div className="text-xs text-base-content/40 flex justify-between px-1">
                {durationMs !== null && <span>时长 {formatTime(durationMs / 1000)}</span>}
                {synthesisMs !== null && <span>耗时 {(synthesisMs / 1000).toFixed(1)}s</span>}
              </div>
            )}
          </div>
      ...
      >
        ...
        {loading ? "合成中，请稍候..." : hasSynthesis ? "重新合成" : "合成对话音频"}
      </button>
